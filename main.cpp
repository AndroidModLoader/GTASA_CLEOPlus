...
extern "C" void OnModLoad()
{
    ...
    CLEO_RegisterOpcode(0xE6C, GET_DAY_NIGHT_BALANCE); // 0E6C=1,get_day_night_balance %1d%
    ...
    CLEO_RegisterOpcode(0xEB8, IS_RADAR_VISIBLE); // 0EB8=0,is_radar_visible
    ...
    CLEO_RegisterOpcode(0xE59, GET_TRAILER_FROM_CAR); // 0E59=2,get_trailer_from_car %1d% trailer %2d%
    CLEO_RegisterOpcode(0xE5A, GET_CAR_FROM_TRAILER); // 0E5A=2,get_car_from_trailer %1d% store_to %2d%
    CLEO_RegisterOpcode(0xE5B, GET_CAR_DUMMY_COORD); // 0E5B=7,get_car_dummy_coord %1d% dummy %2d% world_coords %3d% invert_x %4d% store_to %5d% %6d% %7d% // same as NewOpcodes but this is adapted to VehFuncs
    CLEO_RegisterOpcode(0xE5D, IS_CHEAT_ACTIVE); // 0E5D=1,is_cheat_active %1d%
    CLEO_RegisterOpcode(0xE5E, CHANGE_PLAYER_MONEY); // 0E5E=3,change_player_money %1d% mode %2d% value %3d%
    CLEO_RegisterOpcode(0xE5F, CAR_HORN); // 0E5F=1,car_horn %1d%
    CLEO_RegisterOpcode(0xD4C, GET_STRING_LENGTH); // 0D4C=2,%2d% = string %1s% length // keep NewOpcodes order
    CLEO_RegisterOpcode(0xD4D, COPY_STRING); // 0D4D=2,copy_string %1s% to %2d%
    CLEO_RegisterOpcode(0xE00, GET_CAR_ALARM); // 0E00=2,get_car_alarm %1d% mode_to %2d%
    CLEO_RegisterOpcode(0xE61, SET_CAR_ALARM); // 0E61=2,set_car_alarm %1d% mode %2d%
    CLEO_RegisterOpcode(0xD39, GET_CHAR_MAX_HEALTH); // 0D39=2,%2d% = get_char_max_health %1d% // keep NewOpcodes order
    ...
    CLEO_RegisterOpcode(0xE6F, STREAM_CUSTOM_SCRIPT_FROM_LABEL); // 0E6F=-1,stream_custom_script_from_label %1p%
    CLEO_RegisterOpcode(0xE70, GET_LAST_CREATED_CUSTOM_SCRIPT); // 0E70=1,get_last_created_custom_script %1d%
    ...
    CLEO_RegisterOpcode(0xEB2, GET_OFFSET_FROM_CAMERA_IN_WORLD_COORDS); // 0EB2=6,get_offset_from_camera_in_world_coords %1d% %2d% %3d% store_to %4d% %5d% %6d%
    CLEO_RegisterOpcode(0xD0A, GET_OFFSET_FROM_MATRIX_IN_WORLD_COORDS); // 0D0A=7,store_coords_to %5d% %6d% %7d% from_matrix %1d% with_offsets %2d% %3d% %4d% // keep NewOpcodes order
    CLEO_RegisterOpcode(0xEB4, SET_CAR_COORDINATES_SIMPLE); // 0EB4=4,set_car_coordinates_simple %1d% position %2d% %3d% %4d%
    CLEO_RegisterOpcode(0xEB5, GET_CHAR_DAMAGE_LAST_FRAME); // 0EB5=5,get_char_damage_last_frame %1d% damager %2d% type %3d% part %4d% intensity %5d%
    CLEO_RegisterOpcode(0xEB6, GET_CAR_WEAPON_DAMAGE_LAST_FRAME); // 0EB6=4,get_car_weapon_damage_last_frame %1d% char %2d% type %3d% intensity %4d%
    CLEO_RegisterOpcode(0xEB7, IS_ON_SCRIPTED_CUTSCENE); // 0EB7=0,is_on_scripted_cutscene
    CLEO_RegisterOpcode(0xEBA, GET_MODEL_PED_TYPE_AND_STAT); // 0EBA=3,get_model_ped_type_and_stat %1d% store_to %2d% %3d%
    CLEO_RegisterOpcode(0xEBB, PASS_TIME); // 0EBB=1,pass_time %1d%
    CLEO_RegisterOpcode(0xEBC, GENERATE_RANDOM_INT_IN_RANGE_WITH_SEED); // 0EBC=4,generate_random_int_in_range_with_seed %1d% min %2d% max %3d% store_to %4d%
    CLEO_RegisterOpcode(0xEBD, GENERATE_RANDOM_FLOAT_IN_RANGE_WITH_SEED); // 0EBD=4,generate_random_float_in_range_with_seed %1d% min %2d% max %3d% store_to %4d%
    CLEO_RegisterOpcode(0xEBE, LOCATE_CAMERA_DISTANCE_TO_COORDINATES); // 0EBE=4,locate_camera_distance_to_coordinates %1d% %2d% %3d% radius %4d%
    CLEO_RegisterOpcode(0xEBF, GET_FX_SYSTEM_POINTER); // 0EBF=2,get_fx_system_pointer %1d% store_to %2d%
    CLEO_RegisterOpcode(0xEC0, ADD_FX_SYSTEM_PARTICLE); // 0EC0=14,add_fx_system_particle %1d% coord %2d% %3d% %4d% vel %5d% %6d% %7d% size %8d% brightness %9d% rgba %10d% %11d% %12d% %13d% lastFactor %14d%
    CLEO_RegisterOpcode(0xEC1, IS_FX_SYSTEM_AVAILABLE_WITH_NAME); // 0EC1=1,is_fx_system_available_with_name %1s%
    CLEO_RegisterOpcode(0xEC2, SET_STRING_UPPER); // 0EC2=1,set_string_upper %1s%
    ...
    CLEO_RegisterOpcode(0xECB, GET_CHAR_MOVE_STATE); // 0ECB=2,get_char_move_state %1d% store_to %2d%
    CLEO_RegisterOpcode(0xECC, DONT_DELETE_CHAR_UNTIL_TIME); // 0ECC=2,dont_delete_char_until_time %1d% %2d%
    CLEO_RegisterOpcode(0xECD, DONT_DELETE_CAR_UNTIL_TIME); // 0ECD=2,dont_delete_car_until_time %1d% %2d%
    CLEO_RegisterOpcode(0xECE, GET_TIME_CHAR_IS_DEAD); // 0ECE=2,get_time_char_is_dead %1d% store_to %2d%
    CLEO_RegisterOpcode(0xECF, GET_TIME_CAR_IS_DEAD); // 0ECF=2,get_time_car_is_dead %1d% store_to %2d%
    CLEO_RegisterOpcode(0xED9, SET_CHAR_IGNORE_DAMAGE_ANIMS); // 0ED9=2,set_char_ignore_damage_anims %1d% %2d%
    CLEO_RegisterOpcode(0xEE4, LOCATE_CHAR_DISTANCE_TO_CHAR); // 0EE4=3,locate_char_distance_to_char %1d% char %2d% radius %3d%
    CLEO_RegisterOpcode(0xEE5, LOCATE_CHAR_DISTANCE_TO_CAR); // 0EE5=3,locate_char_distance_to_car %1d% car %2d% radius %3d%
    CLEO_RegisterOpcode(0xEE6, LOCATE_CHAR_DISTANCE_TO_OBJECT); // 0EE6=3,locate_char_distance_to_object %1d% object %2d% radius %3d%
    CLEO_RegisterOpcode(0xEE7, LOCATE_CAR_DISTANCE_TO_OBJECT); // 0EE7=3,locate_car_distance_to_object %1d% object %2d% radius %3d%
    CLEO_RegisterOpcode(0xEE8, LOCATE_CAR_DISTANCE_TO_CAR); // 0EE8=3,locate_car_distance_to_car %1d% car %2d% radius %3d%
    CLEO_RegisterOpcode(0xEE9, LOCATE_OBJECT_DISTANCE_TO_OBJECT); // 0EE9=3,locate_object_distance_to_object %1d% object %2d% radius %3d%
    ...
    CLEO_RegisterOpcode(0xEF6, SET_CAR_OWNED_BY_PLAYER); // 0EF6=2,set_car_owned_by_player %1d% %2d%
    CLEO_RegisterOpcode(0xEF8, GET_MODEL_INFO); // 0EF8=2,get_model_info %1d% store_to %2d%
    CLEO_RegisterOpcode(0xEF9, GET_CAR_ANIMGROUP); // 0EF9=2,get_car_animgroup %1d% store_to %2d%
    CLEO_RegisterOpcode(0xEFA, GET_CHAR_FEAR); // 0EFA=2,get_char_fear %1d% store_to %2d%
    CLEO_RegisterOpcode(0xEFB, IS_CAR_CONVERTIBLE); // 0EFB=1,is_car_convertible %1d%
    ...
    CLEO_RegisterOpcode(0xD27, COPY_MEMORY); // 0D27=3,copy_memory %1d% to %2d% size %3d%
    CLEO_RegisterOpcode(0xD37, WRITE_STRUCT_PARAM); // 0D37=3,write_struct_param %1d% param %2d% value %3d%
    CLEO_RegisterOpcode(0xD38, READ_STRUCT_PARAM); // 0D38=3,%3d% = read_struct_param %1d% param %2d% // keep newOpcodes ordering for SB
    CLEO_RegisterOpcode(0xE6A, MAKE_NOP); // 0E6A=2,make_nop %1d% size %2d%
...
*/
    ...
    // List
    CLEO_RegisterOpcode(0xE72, CREATE_LIST); // 0E72=2,create_list %1d% store_to %2d%
    CLEO_RegisterOpcode(0xE73, DELETE_LIST); // 0E73=1,delete_list %1d%
    CLEO_RegisterOpcode(0xE74, LIST_ADD); // 0E74=2,list_add %1d% value %2d%
    CLEO_RegisterOpcode(0xE75, LIST_REMOVE_VALUE); // 0E75=2,list_remove_value %1d% value %2d%
    CLEO_RegisterOpcode(0xE76, LIST_REMOVE_INDEX); // 0E76=2,list_remove_index %1d% index %2d%
    CLEO_RegisterOpcode(0xE77, GET_LIST_SIZE); // 0E77=2,get_list_size %1d% size %2d%
    CLEO_RegisterOpcode(0xE78, GET_LIST_VALUE_BY_INDEX); // 0E78=3,get_list_value_by_index %1d% index %2d% store_to %3d%
    CLEO_RegisterOpcode(0xE79, RESET_LIST); // 0E79=1,reset_list %1d%
    CLEO_RegisterOpcode(0xE7A, GET_LIST_STRING_VALUE_BY_INDEX); // 0E7A=3,get_list_string_value_by_index %1d% index %2d% store_to %3d%
    CLEO_RegisterOpcode(0xE7B, LIST_ADD_STRING); // 0E7B=2,list_add_string %1d% value %2d%
